...
DynamoRestore.prototype.run = function(finishCallback) {
    ...
    this.on('error', (function(message) {
        ...
        this._restoreWriteCapacity((function (err) {
            if (err) {
                this.emit('error', 'Failed resetting table capacity');
            }
            finishCallback(originalError);
    ...
    this.on('finish', this._restoreWriteCapacity.bind(this, finishCallback));
};

DynamoRestore.prototype._restoreWriteCapacity = function(callback) {
    // Do we need to update write capacity?
    if (this._isCapacityScaled && this.options.writecapacity) {
        return this.dynamodb.updateTable({
            TableName: this.options.table,
            ProvisionedThroughput: {
                ReadCapacityUnits: this.options.readcapacity,
                WriteCapacityUnits: this.options.writecapacity
            }
        }, callback);
    }
...
DynamoRestore.prototype._scaleWriteCapacity = function(callback) {
    var options = this.options;
    if (this._isWriteCapacityScalable()) {
        return this.dynamodb.updateTable({
            TableName: options.table,
            ProvisionedThroughput: {
                ReadCapacityUnits: options.readcapacity,
                WriteCapacityUnits: options.concurrency
            }
        }, function (err) {
            if (err) {
                return callback(err);
            }
            this._isCapacityScaled = true;
            callback();
        }.bind(this));
    }
    return callback();
...
DynamoRestore.prototype._checkTableExists = function(error, data) {
    ...
    if (data.TableNames.indexOf(this.options.table) > -1) {
        ...
        if (this.options.overwrite) {
            ...
            this._scaleWriteCapacity(function (err) {
                if (err) {
                    return this.emit('error', 'Fatal Error. Failed to scale write capacity. ' + err);
                }
...
DynamoRestore.prototype._isWriteCapacityScalable = function() {
    var concurrency = this.options.concurrency;
    var writecapacity = this.options.writecapacity;

    return concurrency && writecapacity && (writecapacity !== concurrency);
};
...
DynamoRestore.prototype._createTable = function(callback) {
    ...
    dynamodb.createTable(params, (function(error, data) {
        ...
        this._isCapacityScaled = this._isWriteCapacityScalable();
